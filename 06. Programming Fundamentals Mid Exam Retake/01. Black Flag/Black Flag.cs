...
namespace _01._Black_Flag
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            double curentPlunder = 0;

            for (int i = 1; i <= days; i++)
            {
                curentPlunder += dailyPlunder;

                if (i % 3 == 0)
                {
                    curentPlunder += 0.5 * dailyPlunder;
                }
                if (i % 5 == 0)
                {
            ...
            if (curentPlunder >= expectedPlunder)
            {
                Console.WriteLine($"Ahoy! {curentPlunder:F2} plunder gained.");
            }
            else
            {
                double percent = (curentPlunder / expectedPlunder) * 100;
